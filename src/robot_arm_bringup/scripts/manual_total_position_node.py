...
class ManualTotalPositionNode(Node):
    # RMD CAN ID 4, 5, 6 구성입니다. Dynamixel을 복구할 때
    # gripper_joint/base_rotate_joint와 각 관절의 rates/limits/입력도 함께 복구합니다.
    JOINTS = ['shoulder_lift_joint', 'elbow_joint', 'wrist_joint']
    ...
    def __init__(self):
        ...
        self.declare_parameter('rates', [0.25, 0.25, 0.25])
        # 최소 각도값
        self.declare_parameter('lower_limits', [0.0, -1.5708, -2.0944])
        # 최대 각도값
        self.declare_parameter('upper_limits', [2.0944, 0.3491, 2.0944])
        # 게임패드에 대응

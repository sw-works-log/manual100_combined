...
find_package(rmd_sdk REQUIRED)

add_executable(your_node
  src/main.cpp
)
target_link_libraries(your_node PUBLIC rmd_sdk::rmd_sdk)
```
...
#include <rmd_sdk/rmd_sdk.hpp>


int main() {
  rmd_sdk::CanDriver driver {"can0"};
  rmd_sdk::ActuatorInterface actuator {driver, 1};
...
In case you installed the package through ROS 2 the shared library will be located inside the `rmd_sdk` package. Therefore you will need to import it with `import rmd_sdk.rmd_sdk_py as rmd`.

For more information you might also inspect the contents of the module inside Python 3 with `help(rmd_sdk_py)`.
